...
{	
	...
	const int fontaa = 4, fontaa2 = fontaa*fontaa;
		...
		class AMove
		{
			int32_t color;
		public:
				AMove (tbal::Color color) : color(*reinterpret_cast<int32_t*>(&color) & 0x00f0f0f0) {}
				void operator () (tbal::Color &dst, int src) const
				{
					if (src)
					{
						int32_t& idst = *reinterpret_cast<int32_t*>(&dst);
						idst &= 0x00f0f0f0;
						idst += ((color-idst)*src)/fontaa2;
					}
				}
		};
	#endif
	...
	void Font::Init (Fixed scale, Fixed width)
	{
		...
		for (int i=0; i<256; ++i)
		{
			...
			if (sx*sy)
			{
				...
				new(&images[i])tblib::array2d<int>(sx, sy);
	...
	int Font::Width(const char *c) const
	{
		int result=0;
		if (c) for(;*c;++c)
			result+=widths[(unsigned char)*c];
		return result;
	}

	int Font::Height(const char *c) const
	{
		(void)c;
		return height*2/3;
	}

	void Font::OutText(const tbal::Bitmap &b, int x, int y, const char *c, tbal::Color color, int alignX, int alignY) const
	{
		x -= Width  (c) * alignX / 2;
		y -= Height (c) * alignY / 2;
		...
		if (c) for(;*c;++c)
		{
			int w = widths[(unsigned char)*c];
			b.Copy(AMove(color), images[(unsigned char)(*c)], x, y);
			x+=w;
		}
	}
};
